...
try{
  ...
  while(market.parent_id !== null){
    ...
    let nextJumpId = market.root_id || market.parent_id || market._id; //first check if there's a precomputed root, then rpeemptively jump to the parent, otherwise set as this node
    ...
    nextJumpId = market.root_id || market.parent_id || market._id; //recompute next jump
